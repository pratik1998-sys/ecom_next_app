...
CategoryRouter.post("/", async (req, res) => {
  try {
    ...
    if (result) {
      res
        .status(201)
        .send({ message: "category added successfully", data: result });
    } else {
...
CategoryRouter.delete("/:categoryID", async (req, res) => {
  try {
    ...
    if (result) {
      res
        .status(201)
        .send({ message: "category deleted successfully", data: result });
    } else {
      console.log("category does not exist");
      res.status(400).send("category does not exist");
    }
...
CategoryRouter.get("/", async (req, res) => {
  try {
    ...
    if (result) {
      res
        .status(200)
        .send({ message: "categories fetched successfully", data: result });
    } else {
